...
/**
 * How one harness is checked and updated. The registry lookup is metadata
 * only; the update itself always runs the harness's own updater command, so
...
const UPDATE_DESCRIPTORS: Partial<Record<HarnessId, HarnessUpdateDescriptor>> = {
  pi: { npmPackage: '@earendil-works/pi-coding-agent', updateArgs: ['update', '--self'] },
}
